...
public class UserService extends ServiceImpl<UserMapper, User> {
    ...
    public UserLoginVo login(UserLoginDto userLoginDto) {
        ...
        if (StringUtil.isNotEmpty(mobile)) {
            //检查手机号是否达到输入错误次数限制,RedisCache从redis中获取错误次数,RedisKeyBuild.createRedisKey构建Redis键
            //redis的格式,业务前缀login_user_mobile_error,多维标识 mobilekey
            ...
            //检查输入的邮箱是否达到输入错误次数限制, redisCache.get从redis中获取输入错误的次数,
            // redisKeyBuild.createRedisKey构建Redis键, String.class表示返回是以字符串的形式
            String errorCountStr = 
            ...
            //构建查询条件. LambdaQueryWrapper<UserEmail>Lambda表达式查询构造器. 针对UserEmail实体
            LambdaQueryWrapper<UserEmail> queryWrapper = Wrappers.lambdaQuery(UserEmail.class)
                    .eq(UserEmail::getEmail, email);//.eq(UserEmail::getEmail,email)表示添加等值条件, UserEmail::getEmail是方法引用,指向email字段
            UserEmail userEmail = userEmailMapper.selectOne(queryWrapper);//执行单条记录查询
    ...
    private GetChannelDataVo getChannelDataByClient(String code){
        ...
        //baseDataClient.getByCode(getChannelDataByCodeDto)调用base-data-service服务的 getByCode方法,是Feign客户端,用于调用基础数据服务
        //getByCode():调用基础数据服务的接口方法, ApiResponse<GetChannelDataVo>标准的API响应格式
