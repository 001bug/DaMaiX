...
-- 进行统计超过普通规则的数量sorted set结构 score当前时间 member 唯一值
...
if (api_rule_type == 2) then
    ...
    for index,depth_rule in ipairs(depthRules)  do
        ...
        -- 将当前时间之前的时间范围的普通规则统计清楚掉, 因为这些过期了
        ...
        if (current_Time >= start_time_window and current_Time <= end_time_window) then
            ...
            if ((rule_trigger_count - depth_rule_threshold) >= 0) then
                ...
                if (redis.call('exists', depth_rule_limit_key) == 0) then -- 检查深度规则限制键是否存在
                    redis.call('set', depth_rule_limit_key, depth_rule_limit_key) -- 设置深度规则限制键,键值是它本身
                    redis.call('expire', depth_rule_limit_key, depth_rule_effective_time) -- 为限制键设置过期时间
                    trigger_result = 1 --触发限制标志
                    trigger_call_Stat = 2  --保存记录类型
                    message_index = index --深度规则索引
                    -- return 短路返回机制. 深度规则一旦触发限制, 就不需要继续检查后续规则. 立即返回结果,提高性能
